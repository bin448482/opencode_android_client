    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ChatTopBar(
            state = ChatTopBarState(
                ...
                selectedModelReference = state.selectedModelReference,
                contextUsage = cachedContextUsage,
                sessionTodos = state.sessionTodos[state.currentSessionId ?: ""] ?: emptyList(),
                aiUsageEnabled = state.aiUsageDashboardUrl.isNotBlank() && state.selectedModel?.providerId in setOf("openai", "zai-coding-plan", "ollama-cloud"),
